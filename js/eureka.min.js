...
for(let target of targets){target.addEventListener('click',()=>{let targetName=target.getAttribute("name")
...
{{-if $enableUtterances}}
function switchUtterancesTheme(theme){try{const message={type:'set-theme',theme:theme,};const utterances=document.querySelector('iframe').contentWindow;utterances.postMessage(message,'https://utteranc.es');}catch{}}
{{-end}}
...
break}
return icon}
function switchDarkMode(e){if(e.matches){document.getElementsByTagName('html')[0].classList.add('dark')
{{}}
{{-if $enableUtterances}}
switchUtterancesTheme('github-dark')
{{-end}}}else{document.getElementsByTagName('html')[0].classList.remove('dark')
{{}}
{{-if $enableUtterances}}
switchUtterancesTheme('github-light')
{{-end}}}}
